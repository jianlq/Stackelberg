...
double CGraph::ORcplex(vector<demand> *eqO){
	...
	for(int ij = 0; ij  < this->m; ij++){		
		...
		for(int d = 0; d < num; d++){
			if(CONSTANT)
				ep1 += x[d][ij] * Link[ij]->latency;  //d之和	
			else
				ep1 += x[d][ij] * Link[ij]->latency*(*eqO)[d].flow;  //d*f之和		
		}	
			delay += ep1;		 		
	}
	...
	if(ORsolver.getStatus() == IloAlgorithm::Infeasible)
		...
		for(int i=0;i<this->m;i++){
			...
			//if(de>0)  // for nash	  但是也可以在nash判断中加入eq[d].flow>0  行94  行73
...
double CGraph::TEbase(vector<demand> &eq){
	IloEnv env; 
	IloModel model(env);
	IloCplex TEsolver(model);
	
	int totalnum=eq.size();
	IloArray<IloIntVarArray> x(env, totalnum);	 // 在遗传中用到的，为了测试方便，改为Num
	for(int d=0; d < totalnum; d++)
		x[d] = IloIntVarArray(env, this->m, 0, 1); 
...
double ORBcplex(CGraph *GOR, vector<demand> &eqO){
	...
	for(int ij = 0; ij  < GOR->m; ij++){		
		...
		for(int d = 0; d < num; d++){
			if(CONSTANT) ep1 += x[d][ij] * GOR->Link[ij]->dist;
				...
				ep1 += x[d][ij] * GOR->Link[ij]->latency*eqO[d].flow;  //delay*f之和			
		}
			delay += ep1;		 
	}
    model.add(IloMinimize(env, delay)); 
   ...
   // friendly 增加的约束
    ...
    for(int d = 0; d < num; d++)
		for(unsigned int i = 0; i < GOR->ver.size(); i++){    // ver.size()为顶点数,ver[i]为顶点编号
			IloExpr constraint(env);
	...
	if(ORBsolver.getStatus() == IloAlgorithm::Infeasible)
		...
		for(int i=0;i<GOR->m;i++){
			double de=0;
			for(int d=0;d<num;d++){
				if(ORBsolver.getValue(x[d][i])>1e-6)
