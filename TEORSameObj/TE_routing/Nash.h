...
//约束2 带宽约束
  ...
  IloNumVarArray cost(env,this->m,0.0 ,IloInfinity); 
	if(SAMEDELAY){
		if(!CONSTANT){
	IloExpr res(env);
	for(int i=0;i<this->m;i++){
	...
	model.add(cost[i]>=load/(this->Link[i]->capacity-3.0));   // [1,1/3]
	model.add(cost[i]>=3.0*load/(this->Link[i]->capacity-3.0)-(2.0/3.0));  //[1/3,2/3]
	model.add(cost[i]>=10.0*load/(this->Link[i]->capacity-3.0)-(14.0/3.0)); // [2/3,9/10]
	model.add(cost[i]>=70.0*load/(this->Link[i]->capacity-3.0)-(163.0/3.0));  //[9/10,1]
