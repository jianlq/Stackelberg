...
double ORdictor(CGraph *G,vector<demand> &eq,int ornum){    /////  eq的前ornum(GOR->m)个先装Overlay的需求  ////////
	...
	model.add(cost[i]>=3.0*load/(G->Link[i]->capacity-1.0)-(2.0/3.0));  //[1/3,2/3]
	model.add(cost[i]>=10.0*load/(G->Link[i]->capacity-1.0)-(14.0/3.0)); // [2/3,9/10]
	model.add(cost[i]>=70.0*load/(G->Link[i]->capacity-1.0)-(163.0/3.0));  //[9/10,1]
...
double TEdictor(CGraph *G,vector<demand> &eq,CGraph *GOR){    ////////   eq的前面装eqOR  //////////
	...
	////////常数延迟
		...
		for(int d=0;d<GOR->m;d++){  
			for(int ij=0;ij<G->m;ij++){
			   if(G->Link[ij]->load>=G->Link[ij]->capacity) lat += 5.0;
