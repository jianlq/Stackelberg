...
double ORdictor(CGraph *G,vector<demand> &eq,int ORNUMDEMAND){    /////  eq的前ORNUMDEMAND个先装Overlay的需求  ////////
    IloEnv env;
    IloModel model(env);
	...
	if(TEsolver.getStatus() == IloAlgorithm::Infeasible)
		...
		for(int d=0;d<GOR->m;d++){   ////GOR->m个Overlay的需求
			for(int ij=0;ij<G->m;ij++){
				...
				lat += TEsolver.getValue(x[d][ij])*del*eq[d].flow; // 1/(C-x)  * x1
			}
		}
		G->delay=lat;
		...
		///////折算到Overlay计算
		 //延迟非常数  1.0/(C-x+1)	
		for(int ij=0;ij<G->m;ij++){
			if( G->Link[ij]->load>=G->Link[ij]->capacity) G->Link[ij]->latency=1.0;
			else G->Link[ij]->latency=1.0/(G->Link[ij]->capacity-G->Link[ij]->load);  
		}
		//计算OR边延迟
		  for(int m=0;m<GOR->m;m++){	
			...
			for(int d=0;d<num;d++)
			{
				if(GOR->Link[m]->tail==eq[d].org && GOR->Link[m]->head==eq[d].des && (eq[d].flow>0) ){ //OR边在需求中
					...
					flag=true;
					for(int ij=0;ij<G->m;ij++)
						dmin += TEsolver.getValue(x[d][ij])*G->Link[ij]->latency; 
			      }
				}
			if(flag==true){
		  ...
		  for(int m=0;m<GOR->m;m++){	
			...
			for(int d=0;d<num;d++)
			{
				if(GOR->Link[m]->tail==eq[d].org && GOR->Link[m]->head==eq[d].des && (eq[d].flow>0) ){ //OR边在需求中
					...
					flag=true;
					for(int ij=0;ij<G->m;ij++){
						if(G->Link[ij]->load>=G->Link[ij]->capacity) dmin += 100.0;
						else dmin += TEsolver.getValue(x[d][ij])*G->Link[ij]->dist; 
					}
			      }
				}
			if(flag==true){
